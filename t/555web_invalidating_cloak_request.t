...
my $req = $schema->resultset('CloakChange')->create({
        cloak        => 'group0/user0',
        group        => $group,
        changed_by   => $user,
        target       => $user,
...
$ua->get_ok(
    'http://localhost/json/admin/approve_cloak',
);

$ua->content_contains('group0/user0', 'Request is there.');
...
$ua->content_lacks('group0/user0', 'Inactive request is not there');
